...
int ospfs_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, long arg)
{
...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		 ...
		 * EXERCISE: Your code here */
		 ...
		 // if reached end of directory blockno will return 0
		...
		// if the file is an empty directory try the next one
		if (od->od_ino == 0)
		{
			f_pos++;
			continue;
		...
		// otherwise get the inode of the file the directory points to
...
// decrease nwrites_to_crash when function succeeds
static uint32_t
allocate_block(void)
{
	...
	while(current_block < os_nblocks)
	{
		...
		if (bitvector_test(bitmap_block, current_block) == 1)
		{
			// mark block as non-empty and return
			if (nwrites_to_crash != 0)
				bitvector_clear(bitmap_block, current_block);
			if (nwrites_to_crash > 0)
				nwrites_to_crash--;
...
free_block(uint32_t blockno)
{
	if (nwrites_to_crash == 0)
		return;
	// find last inode blockno
	uint32_t last_inode_blockno = ospfs_super->os_firstinob + ospfs_super->os_ninodes/OSPFS_BLKINODES;
	// if blockno greater than last inode no, then it is a valid blockno
	if(blockno > last_inode_blockno)
	{
		// get bitmap block corresponding to blockno
		void * bitmap_block = ospfs_block(OSPFS_FREEMAP_BLK + blockno/OSPFS_BLKBITSIZE);
		// set blockno bit to free
		if (nwrites_to_crash != 0)
...
{
	// if b is larger than the NDIRECT and NINDIRECT, a doubly indirect block is needed
...
{
	uint32_t offset;
	// if the file requires to be put in doubly indirect block
	// return offset of the indirect block inside the doubly indirect block
	if (b >= OSPFS_NDIRECT + OSPFS_NINDIRECT)
	{
		offset = b - (OSPFS_NDIRECT + OSPFS_NINDIRECT);
		return offset/OSPFS_NINDIRECT;
	}
...
{
	// get offset of whatever access block b is contained in
	if (b >= OSPFS_NDIRECT + OSPFS_NINDIRECT)
	{
		uint32_t offset = b - (OSPFS_NDIRECT + OSPFS_NINDIRECT);
		return offset % OSPFS_NINDIRECT;
	}
	else if (b >= OSPFS_NDIRECT)
...
add_block(ospfs_inode_t *oi)
{
	...
	// need a new doubly indirect block
	...
	if (indir2_index(n) == 0)
	{
		// check if doubly indirect block was just allocated
		if (allocated[1] != 0)
			oi->oi_indirect2 = allocated[1];

		uint32_t *indirect2_block = ospfs_block(oi->oi_indirect2);
		// if direct block was just allocated
		if (allocated[0] != 0)
			indirect2_block[indir_index(n)] = allocated[0];

		uint32_t *indirect_block = ospfs_block(indirect2_block[indir_index(n)]);
		// assign new block to indirect block
		indirect_block[direct_index(n)] = newblock;
	}
	...
	else if (n >= OSPFS_NDIRECT)
	{
		...
		oi->oi_direct[direct_index(n)] = newblock;
...
remove_block(ospfs_inode_t *oi)
{
	// current number of blocks in file
	uint32_t n = ospfs_size2nblocks(oi->oi_size);
	uint32_t dir_index = direct_index(n-1);
	uint32_t ind_index = indir_index(n-1);

	// if size fits in direct blocks it is simple
	if (n <= OSPFS_NDIRECT)
	{
	...
	// otherwise if it fits in first indirect block
	else if (n <= OSPFS_NDIRECT + OSPFS_NINDIRECT)
	{
		...
		// check if first block of indirect block was deleted
		if(dir_index == 0)
		{
	...
	// otherwise a doubly indirected block was used
	else
	{
		...
		if (indirect2_block[ind_index] == 0)
			return -EIO;

		uint32_t *indirect_block = ospfs_block(indirect2_block[ind_index]);
		free_block(indirect_block[dir_index]);
		indirect_block[dir_index] = 0;
		// check if first block in indirect block was deleted
		if (dir_index == 0)
		{
			free_block(indirect2_block[ind_index]);
			indirect2_block[ind_index] = 0;
			// check if first indirect block in doubly indirect block was deleted
			if (ind_index == 0)
			{
...
change_size(ospfs_inode_t *oi, uint32_t new_size)
{
	...
	// add blocks until correct size with error checkings
	...
	while (ospfs_size2nblocks(oi->oi_size) > ospfs_size2nblocks(new_size)) {
		...
		// if error, revert back to old size
		if (r != 0)
			oi->oi_size = old_size;
		else
			oi->oi_size = new_size;
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		if (count - amount + offset > OSPFS_BLKSIZE)
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	if (nwrites_to_crash != 0){
		...
		if (*f_pos + count > oi->oi_size)
		{
			err = change_size(oi, *f_pos + count);
			if (err < 0)
				goto done;
		}
		...
		while (amount < count && retval >= 0) {
			...
			err = copy_from_user(data + offset, buffer, n);
			if (err != 0)
				return -EFAULT;
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	// iterate through all directories in the block, changing the index accordingly
	...
	// adjust size to accomodate new directory
	r = add_block(dir_oi);
	if (r < 0)
		return ERR_PTR(r);

	// get the start of the directory before the change in size
	dir_entry = ospfs_inode_data(dir_oi, dir_index);
	return dir_entry;
}
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	// otherwise create a new blank directory entry
	od = create_blank_direntry(dir_oi);
	if (IS_ERR(od))
		return PTR_ERR(od);

	// check lenght of destianation name
	if (dst_dentry->d_name.len > OSPFS_MAXNAMELEN)
		return -ENAMETOOLONG;

	// increase the number of links to the file
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	while(entry_ino < ospfs_super->os_ninodes)
	{
		...
		// if an inode was found and it has no links
	...
	// initialize new inode's attributes
	if (nwrites_to_crash != 0)
	{
		new_inode->oi_nlink++;
		new_inode->oi_size = 0;
		...
		memset(new_inode->oi_direct, 0, sizeof(uint32_t) * OSPFS_NDIRECT);
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	ospfs_symlink_inode_t *new_sym;

	// check length of symlink and directory
	...
	// find an inode with no links
	while(entry_ino < ospfs_super->os_ninodes)
	{
		new_sym = (ospfs_symlink_inode_t*)ospfs_inode(entry_ino);
		if (new_sym && new_sym->oi_nlink == 0)
			break;
		entry_ino++;
	}
	...
	if (nwrites_to_crash != 0)
	{
		// change inode's attributes accordingly including size and type
		new_sym->oi_ftype = OSPFS_FTYPE_SYMLINK;
		new_sym->oi_nlink = 1;
		new_sym->oi_size = strlen(symname);
		strncpy(new_sym->oi_symlink, symname, new_sym->oi_size);
		new_sym->oi_symlink[new_sym->oi_size] = 0;

		// fill in directory attributes from dentry
...
int find_char_index_in_array(char *arr, char c)
{
	int i = 0;
	if (arr == NULL)
		return -1;
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	// find indices of chars that separate conditional
	int question_index = find_char_index_in_array(oi->oi_symlink, '?');
	int colon_index = find_char_index_in_array(oi->oi_symlink, ':');

	// buffers used for storing condition and destination
	char* condition;
	char* destination;

	// this is a condition symlink
	if (question_index != -1 && colon_index != -1)
	{
		// allocate room for buffer
		condition = kmalloc(oi->oi_size, GFP_ATOMIC);
		destination = kmalloc(oi->oi_size, GFP_ATOMIC);

		// fill in the conditional buffer
		for (i = 0; i < question_index; i++)
			condition[i] = oi->oi_symlink[i];
		condition[i] = 0;

		// if we are the root user
		if(current->uid == 0 && strcmp(condition, "root") == 0)
		{
			// fill in first destination buffer
			for (i = question_index + 1; i < colon_index; i++)
			{
				destination[k] = oi->oi_symlink[i];
				k++;
			}
			destination[k] = 0;
		...
		// otherwise choose the other name
		else
		{
			// fill in the second destination buffer
	...
	// otherwise it is a normal symlink
	else
...
static struct file_operations ospfs_reg_file_ops = {
	...
	.read		= ospfs_read,
	.write		= ospfs_write,
	.ioctl 		= ospfs_ioctl // Design problem
};
...
MODULE_AUTHOR("Alan Covarrubias");
